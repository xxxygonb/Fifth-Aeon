...
export class SummonUnits extends TriggeredMechanic {
    ...
    public getText(card: Card, game: Game) {
        if (this.count === 1) {
            return tf('Summon a {name}.', { name: this.name });
        }
        return tf('Summon {n} {name}.', {
            n: this.count,
            name: this.name
        });
    }
        ...
        if (game) {
            return tf(
                'Play a {name} for each {factor} units in any crypt [dynamic]({n})[/dynamic].',
                {
                    name: this.name,
                    factor: this.factor,
                    n: this.getUnitCount(card, game)
                }
            );
        } else {
            return tf(
                'Play a {name} for each {factor} units in any crypt (rounded down).',
                { name: this.name, factor: this.factor }
            );
        }
    }
}
...
export class SummonUnitOnDamage extends Mechanic {
    ...
    public getText(card: Card) {
        return tf('Whenever this damages your opponent summon a {name}.', {
            name: this.name
        });
    }
