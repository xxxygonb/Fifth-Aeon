...
export class TipService {
    ...
    public setUsername(username: string) {
        this.startGameTip = this.i18n.tr(
    ...
    public playTip(tip: TipType): boolean {
        ...
        if (tip === TipType.StartGame) {
            this.announce(this.startGameTip);
        } else {
            this.announce(this.i18n.tr(tipText.get(tip) || ''));
        }
    ...
    public cannotBlockTargetTip(blocker: Unit, attacker: Unit, game: Game) {
        if (!attacker.isAttacking()) {
            ...
            this.announce(
                this.i18n.tr('{name} is unblockable.', {
            ...
            this.announce(
                this.i18n.tr('Aquatic units may only block other aquatic units.')
            );
        } else {
            this.announce(
                this.i18n.tr('{blocker} cannot block {attacker} due to a special effect.', {
    ...
    public cannotBlockTip(blocker: Unit, game: Game) {
        if (blocker.isExhausted()) {
            this.announce(this.i18n.tr('Exhausted units can not block.'));
        } else {
            this.announce(
                this.i18n.tr('That unit can not block due to a special effect.')
            );
        }
    }
    ...
    public cannotAttackTip(unit: Unit, game: Game) {
        if (!unit.isReady()) {
            this.announce(
                this.i18n.tr('Units cannot attack the turn they are played.')
            );
        } else if (unit.isExhausted()) {
            this.announce(this.i18n.tr('Exhausted units cannot attack.'));
        } else if (!game.canTakeAction()) {
            this.announce(this.i18n.tr('You must wait for a choice to be made.'));
        } else {
            this.announce(
                this.i18n.tr('That unit cannot attack due to a special effect')
            );
        }
    }
        ...
        if (player !== game.getCurrentPlayer()) {
            ...
            this.announce(
                this.i18n.tr(
                    ...
                    {
                        verb: this.i18n.tr(verb),
                        need: enchantment.getModifyCost().getNumeric(),
                        have: player.getPool().getNumeric()
                    }
                )
            );
        }
    ...
    public cannotPlayTip(playerNo: number, game: Game, card: Card) {
        ...
        if (game.getCurrentPlayer().getPlayerNumber() !== playerNo) {
            this.announce(
                this.i18n.tr('You can only play cards on your own turn.')
            );
        } else if (!player.getPool().meetsReq(card.getCost())) {
            const diff = card.getCost().difference(player.getPool());
            this.announce(
                this.i18n.tr('You need {need} to play {card}.', {
                    need: diff
                        .map(change =>
                            this.i18n.tr('{n} more {res}', {
                                n: change.diff,
                                res: t(change.name)
                            })
                        )
                        .join(this.i18n.tr('and')),
                    card: card.getName().replace(/\./g, '')
                })
            );
        } else if (
            ...
            this.announce(
                this.i18n.tr('Your board is too full to play a unit.')
            );
        } else if (
            ...
            this.announce(
                this.i18n.tr('Your board is too full to play an enchantment.')
            );
        } else if (
            ...
            this.announce(
                this.i18n.tr('You don\'t have any units to attach that item to.')
            );
        } else if (!card.getTargeter().isTargetable(card, game)) {
            this.announce(
                this.i18n.tr('There are no valid targets for {name}.', {
                    name: card.getName()
                })
            );
        } else {
