...
export class P2PDialogComponent {
    ...
    public async start() {
        ...
        if (this.signalingMethod === 'manual') {
            ...
            this.manualLog = this.role === 'host'
                ? this.i18n.tr('Generating offer...')
                : this.i18n.tr('Waiting for offer...');

        } else {
            this.signaling = new FirebaseSignalingService();
            this.firebaseStatus = this.i18n.tr('Connecting...');
            ...
            try {
                ...
                this.firebaseStatus = this.role === 'host'
                    ? this.i18n.tr('Waiting for opponent...')
                    : this.i18n.tr('Connected!');

                this.client.startP2PGame(this.signaling!, this.role === 'host');
            } catch (e) {
                this.firebaseStatus = this.i18n.tr('Error: ') + e;
            }
        }
    }
    ...
    public processManualInput() {
        if (this.manualSignaling && this.manualAnswer) {
            ...
            this.manualLog = this.i18n.tr('Signal processed.');
        }
    }
