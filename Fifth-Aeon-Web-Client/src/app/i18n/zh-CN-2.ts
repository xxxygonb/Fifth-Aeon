...
export const zhCNUI2: Record<string, string> = {
    ...
    // 格式：'名称 · 一句话说明'，让玩家不看文档也能选对机制
    // Mechanic IDs
    AbominationConsume: '憎恶吞噬 · 吞噬墓地单位并夺取属性',
    Annihilate: '湮灭 · 消灭目标，且不触发其亡语',
    AugarCard: '占卜 · 查看并调整即将抽到的牌',
    BiteDamage: '撕咬 · 造成伤害，并回复等量生命',
    BuffTarget: '属性增益 · 令目标获得 +X/+Y',
    CannotBeDiminished: '不可削弱 · 对手无法降低此附魔力量',
    CannotBeEmpowered: '不可强化 · 无法增加此附魔力量',
    CannotBlock: '不能阻挡 · 此单位无法进行阻挡',
    ChangePower: '力量增减 · 增加或减少附魔的力量',
    CurePoison: '解毒 · 移除目标身上的中毒效果',
    DamageOnBlock: '阻挡反击 · 阻挡时先对攻击者造成伤害',
    DamageSpawnOnKill: '击杀召唤 · 击杀单位后召唤衍生体',
    DealDamage: '造成伤害 · 对目标造成 X 点伤害',
    DealResourceDamage: '按资源伤害 · 造成等同你资源数量的伤害',
    DeathCounter: '死亡计数 · 每死亡一次消耗一层护盾',
    Discard: '弃牌 · 弃掉指定数量的手牌',
    DiscardOnDamage: '受伤弃牌 · 此单位受伤时你弃一张牌',
    DrainPower: '吸取力量 · 从其他附魔中汲取力量',
    DrainPowerIntoStats: '力量转属性 · 汲取附魔力量化为自身属性',
    DrawCard: '抽牌 · 抽取卡牌',
    DrawCardsFromUnit: '按属性抽牌 · 抽取等同目标属性÷N 的牌',
    EnchantmentSummon: '附魔召唤 · 附魔入场时召唤衍生单位',
    ForceField: '力场 · 你受到的伤害由附魔力量承担',
    FriendlyLordship: '友方统领 · 光环持续增益友方同类',
    GainLife: '获得生命 · 回复玩家的生命值',
    GainResource: '获得资源 · 获得能量与阵营资源',
    GrantAbility: '授予能力 · 将某关键词赋予目标单位',
    ImprisonTarget: '囚禁 · 目标无法行动，直至囚禁者离场',
    ImprisonTemporarily: '临时囚禁 · 目标短暂无法行动',
    KillTarget: '消灭 · 直接消灭目标单位',
    Lordship: '统领光环 · 持续增益场上同类单位',
    MindControl: '心灵控制 · 夺取目标单位的控制权',
    NotUnitTypeLordship: '异类统领 · 增益非该种族的友方单位',
    Peek: '窥视 · 查看对手的手牌',
    PoisonImmune: '免疫中毒 · 不受中毒与剧毒影响',
    PoisonTarget: '施毒 · 目标中毒（每回合 -1/-1）',
    PreventAllDamage: '完全格挡 · 防止你受到的所有伤害',
    RefreshTarget: '治疗刷新 · 治疗目标至满血并解除疲惫',
    RemovePower: '移除力量 · 降低目标附魔的力量',
    ReturnFromCrypt: '墓地复活 · 将单位从墓地召回场上',
    ShuffleIntoDeck: '洗回牌库 · 将目标单位洗回其卡组',
    SleepTarget: '催眠 · 目标沉睡，数回合内无法行动',
    SpyPower: '间谍之力 · 依据间谍单位获得增益',
    SummonUnitForGrave: '按墓地召唤 · 每有 N 个墓地单位召唤一个',
    SummonUnitOnDamage: '受击召唤 · 受到伤害时召唤衍生单位',
    SummonUnits: '召唤单位 · 召唤指定数量的衍生单位',
    TransformDamaged: '受击变形 · 首次受伤后变形为另一单位',
    UnitTypeLordshipAll: '同类统领 · 增益该种族的全部单位',
    UnitTypeLordshipExclusive: '同类统领 · 增益同类单位（不含自身）',
    UnitTypeLordshipInclusive: '同类统领 · 增益同类单位（含自身）',
    WebTarget: '蛛网缠身 · 疲惫目标并移除其飞行',
    WinIfHighLife: '高生命获胜 · 生命达到条件立即获胜',

    // Trigger IDs
    // （Play/Affinity/Serenity/Dawn/Dusk/Cycle 在上方关键词区块已有简短翻译，
    //   编辑器下拉的带说明版本由 mechanic-editor 组件内的 TRIGGER_LABELS 提供）
    Death: '亡语 · 此单位死亡时',
    FriendlyBioUnitEntersPlay: '友方生物入场时',
    FriendlyMechUnitEntersPlay: '友方机械入场时',
    FriendlyUnitEntersPlay: '友方单位入场时',
    LethalStrike: '致命一击 · 造成致命伤害后',
    OwnerAttacked: '你被攻击时',
    OwnerDrawsUnit: '你抽到单位牌时',
    SoulReap: '亡魂收割 · 任意其他单位死亡时',
    ...
    'Move up (effects resolve top to bottom)':
        '上移（效果按从上到下顺序结算）',
    ...
    'Card Tutorial': '卡牌添加教程',

    // Targeter IDs
    AllEnchantments: '所有附魔',
    AllOtherUnits: '其他所有单位（不含自身）',
    AllPermanents: '所有在场卡牌',
    AllPlayers: '双方玩家',
    AllUnits: '所有单位（双方）',
    BiologicalUnit: '生物单位（非机械）',
    CurePoisonTargeter: '中毒的单位',
    DamagedUnit: '已受伤的单位',
    Enemies: '敌方全体单位',
    EnemyPlayer: '对手玩家',
    EnemyUnit: '一个敌方单位',
    EnemyUnits: '敌方全部单位',
    Everyone: '所有单位与玩家',
    FriendlyBiologicalUnits: '友方生物单位全体',
    FriendlyUnit: '一个友方单位',
    FriendlyUnits: '友方全部单位',
    FriendlyUnitsOfType: '友方同种族单位',
    FriendlyVehicleOrStructure: '友方载具或建筑',
    Friends: '友方全体',
    LifeLessUnit: '攻击力为 0 的单位',
    LifeLessUnits: '攻击力为 0 的单位全体',
    MechanicalUnit: '机械单位',
    OwningPlayer: '这张卡的拥有者',
    PoisonableUnit: '可被中毒的单位',
    PoisonableUnits: '可被中毒的单位全体',
    SelfTarget: '自身（这张卡）',
    SingleUnit: '任意单个单位',
    SleepableUnit: '可被催眠的单位',
    TriggeringUnit: '触发事件的单位',
    UnitOfType: '指定种族的单位',
    UnitsNotOfType: '非该种族的单位全体',
    UnitsOfType: '该种族的单位全体',
    UnitsOfTypeAsTarget: '该种族单位（作为目标）',
    UnitWithAbility: '拥有指定能力的单位',
    Untargeted: '无目标（不指定对象）'
};
