...
export class Resource {
    ...
    public asSentence() {
        ...
        if (reqs.length === 0) {
            return tf('costs {n} energy.', { n: this.numeric });
        } else {
            return tf('costs {n} energy. It also requires {reqs}.', {
                n: this.numeric,
                reqs: properList(
                    reqs.map(pair =>
                        tf('{n} {type}', { n: pair[1], type: t(pair[0]) })
                    )
                )
            });
        }
    ...
    public asListDesc() {
        ...
        const items = [tf('{n} energy', { n: this.numeric })].concat(
            reqs.map(pair =>
                tf('{n} {type}', { n: pair[1], type: t(pair[0]) })
            )
        );
