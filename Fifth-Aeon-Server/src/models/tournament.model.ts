...
class TournamentModel {
    ...
    public async getTeamInformation(user: UserData): Promise<TeamData> {
        if (!(await this.isMemberOfTeam(user))) {
            return Promise.reject({
                problem: tsrv("Your not on a team")
            });
        }
    ...
    private async getLedTeam(user: UserData) {
        ...
        if ((teamId.rowCount ?? 0) < 1) {
            return Promise.reject({
                problem: tsrv("You are not the leader of a team.")
            });
        }
    ...
    public async joinTeam(user: UserData, code: string) {
        ...
        if (teamExists.rowCount === 0) {
            return Promise.reject({
                problem: tsrv("No team exists with that join code.")
            });
        }
    ...
    public async exitTeam(user: UserData) {
        if (!(await this.isMemberOfTeam(user))) {
            return Promise.reject({
                problem: tsrv("Cannot leave team your not in one")
            });
        }
    ...
    private async getActiveTournament(): Promise<string> {
        ...
        if ((result.rowCount ?? 0) === 0) {
            return Promise.reject({
                problem: tsrv("No Active tournament")
            });
        } else if ((result.rowCount ?? 0) > 1) {
            return Promise.reject({
                problem: tsrv("Multiple Active tournaments")
            });
        }
